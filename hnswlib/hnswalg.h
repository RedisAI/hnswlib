...
namespace hnswlib {
    ...
    public:
        ...
        HierarchicalNSW(SpaceInterface<dist_t> *s, size_t max_elements, size_t M = 16, size_t ef_construction = 200, size_t ef = 10, size_t random_seed = 100) :
            ...
            // -----4------ | -----4*M0----------- | ----8*M0------------| -----8-------------- | ---32---- | ----8-- |
            // <links_len>  | <link_1> <link_2>... | <dist_1> <dist_2>...| <incoming_links_set> |   <data>  | <label> |
            ...
            // -----4------ | -----4*M-------------- | ------8*M----------- | ----8----------------|
            // <links_len>  | <link_1> <link_2> ...  | <dist_1> <dist_2>... | <incoming_links_set> |
        ...
        tableint mutuallyConnectNewElement(tableint cur_c, CandidatesQueue<dist_t> &top_candidates, int level) {
            ...
            for (size_t idx = 0; idx < selectedNeighbors.size(); idx++) {
                ...
                if (sz_link_list_other < Mcurmax) {
                    ...
                    // finding the "weakest" element to replace it with the new one with the heuristic:
                    CandidatesQueue<dist_t> candidates;
                    candidates.emplace(dist_to_selected_neighbors[idx], cur_c);

                    for (size_t j = 0; j < sz_link_list_other; j++) {
                        // this is optimized by saving the distances cache.
        ...
        void repairConnectionsForDeletion(tableint element_internal_id, tableint neighbour_id,
          tableint *neighbours_list, tableint *neighbour_neighbours_list, int level) {
            ...
            // put the deleted element's neighbor's neighbours in the candidates.
            CandidatesQueue<dist_t> candidates;
            ...
            // add the deleted element's neighbours in the candidates.
            ...
            for (size_t j = 0; j < neighbours_count; j++) {
                // Don't put the neighbor itself in the candidates, nor nodes that are already in the candidates set.
